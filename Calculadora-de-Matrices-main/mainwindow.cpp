        ...
        if(d.en_checked()){
            for(int i=0;i<fa;i++){
                ...
                for(int j=0;j<ca;j++){
                    ...
                    ui->A->setItem(i,j,new QTableWidgetItem(QString::number(tf[x].toDouble() / e)));
                    if(x == tam-1){
                        x = 0;
                    }
                    else{
...
void MainWindow::on_actionA_con_B_triggered(){
    Llenar();

    pye o(this);

    o.PasarDatos(ui->A,ui->B);
    o.A_con_B();

    o.exec();
}

double MainWindow::Determinante(double a[][100]){
    nxn = fa;
    double d[100][102];
    double aux[100][100];
    double Det;
...
void MainWindow::on_actionResolver_triggered()
{
   ...
   for(int i=0;i<fa;i++){
       ...
       v[i] = ui->vResultados->item(i,0)->text().toDouble();
   }
   ...
   if(fa != ca ){
       ...
       //determinante
       resultado r(this);
       r.setWindowIcon(QIcon(":/new/prefix1/iconos/resolver.png"));
