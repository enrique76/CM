...
void pye::PasarDatos(QTableWidget *a, QTableWidget *b){
    PasarDatos(a);

    this->b = new double*[b->rowCount()];

    for(int i=0;i<b->rowCount();i++){
        datos[i] = new double[b->columnCount()];
    }
    ...
    for(int i=0;i<f;i++){
        for(int j=0;j<c;j++){
            datos[i][j] = b->item(i,j)->text().toDouble();
        }
    }
...
void pye::A_con_B(){
    ...
    setWindowTitle("Histograma A con respecto a B");


//1. Ordenar el vector con Shell
...
// 2. Ancho del QTableWidget donde tabularemos A y B

ui->h->setColumnWidth(0,(ui->h->width()/4));
ui->h->setColumnWidth(1,(ui->h->width()/4));
ui->h->setColumnWidth(2,(ui->h->width()/4));
ui->h->setColumnWidth(3,(ui->h->width()/4));

// graficas lineas

QLineSeries *DatosA = new QLineSeries();
QLineSeries *DatosB = new QLineSeries();
...
chart->setTheme(QChart::ChartThemeDark);
chart->legend()->hide();
chart->setTitle("Histograma A con respecto a B");
chart->createDefaultAxes();

QChartView *plot = new QChartView(chart);
plot->setParent(ui->baseAB);
plot->setRenderHint(QPainter::Antialiasing);
